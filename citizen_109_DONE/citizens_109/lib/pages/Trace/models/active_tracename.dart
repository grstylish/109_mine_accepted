...
class ActiveTraceName {
  ...
  factory ActiveTraceName.parseMap(Map object) {
    String deadline = object['deadline'];
    print("DEDE 1  $deadline");
    if (deadline != 'None') deadline += ' Z';

    return ActiveTraceName(
      ...
      statuses: Statuses.parseList(
        object['statuses'],
      ),
    );
  }

  static List<ActiveTraceName> parseList(List objectList) {
    List<ActiveTraceName> list = [];
    for (Map item in objectList) list.add(ActiveTraceName.parseMap(item));
    return list;
  }
